...
First we will generate a toy dataset on which we may test each method. We will 
...
Having obtained a vector of 1000 estimates of $\hat{\theta}$, we may plot the 
...
Sample splitting entails dividing the data into two groups, say $S_1$ and $S_2$ 
...
To start, we will repeat the above DML simulation with and without sample 
splitting. When not using sample splitting, both the function approximation and 
effect estimation will be done using the entire dataset. The following plot 
shows the distributions of $\hat{\theta}$ for each method:

```{r, eval=F, echo=F}
theta.split <- numeric(n_sim)
theta.nosplit <- numeric(n_sim)
...
for (i in 1:n_sim) {
  ...
  theta.split[i] <- with(s2, 
                         mean(V.hat.ss * (Y - g.hat.ss)) / mean(V.hat.ss * D))
  ...
  theta.nosplit[i] <- with(data, 
                           mean(V.hat.ns * (Y - g.hat.ns)) / mean(V.hat.ns * D))
}
```

```{r, echo=F}
load("theta_split.RData")
load("theta_nosplit.RData")

theta.df <- tibble(Yes=theta.split, No=theta.nosplit) %>%
...
ggplot(data=theta.df) +
  ...
  geom_vline(aes(xintercept=theta, color="black")) +
  labs(x=TeX("$\\hat{\\theta}$"), y="Density",
       title=TeX("$\\hat{\\theta}$ Distribution with/without Sample Splitting"),
       color="") +
  scale_color_manual(values=c("True Value"="black")) +
  scale_fill_manual(values=c("salmon", "lightblue")) + 
  theme_bw()
...
It is clear from these distributions that bias is not avoided by double machine 
learning when we neglect sample splitting. We therefore require that sample 
splitting is used in the DML process.

### Cross-Fitting

Cross-fitting is one more technique that we may apply in double ML to achieve 
...
Finally, we may compare the methods of cross-fitting, sample splitting, and 
no sample splitting by evaluating the distributions of the estimates they 
generate.
...
theta.df <- tibble(
  `Cross-fitting`=theta.cross, 
  `Sample Splitting`=theta.split, 
  `No Sample Splitting`=theta.nosplit
) %>%
