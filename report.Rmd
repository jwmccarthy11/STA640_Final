...
We do so for our simulated dataset below using random forest regressors.Having 
obtained a vector of 1000 estimates of $\hat{\theta}$, we may plot the 
...
and the non-linear nuisance functions are defined to be
...
We may now visualize the results for the simple non-linear nuisance functions.
...
with the benefits of sample-splitting being realized in particular for 
high-dimensional covariates.

### Complex Nuisance

Next we will investigate the benefits of sample splitting for more complex
nuisance functions. In this section we will only look at a partial-linear 
model since the previous section revealed little difference between the 
treatment effect estimates for a partial-linear and fully-linear data 
